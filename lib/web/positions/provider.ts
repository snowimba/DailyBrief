...
function shanghaiTime(now = new Date()): { day: string; hhmm: string; weekday: number } {
  // Convert to Asia/Shanghai (UTC+8). DST not in effect in CN.
  const sh = new Date(now.getTime() + 8 * 60 * 60 * 1000);
  const day = sh.toISOString().slice(0, 10);
  const hh = String(sh.getUTCHours()).padStart(2, "0");
  const mm = String(sh.getUTCMinutes()).padStart(2, "0");
  return { day, hhmm: `${hh}:${mm}`, weekday: sh.getUTCDay() };
}

function isTradingHours(now = new Date()): boolean {
  const { hhmm, weekday } = shanghaiTime(now);
...
interface SnapshotPayload {
  ...
  industries: Record<string, number>;       // industry → market value
...
export async function buildSnapshot(): Promise<SnapshotPayload> {
  ...
  // Announcements fetched in parallel for all positions, with a per-symbol
  // 30-min cache (see announcements.ts).
  const annMap = await fetchAnnouncements(positions.map((p) => p.symbol));
...
export const positionsProvider: TabProvider = {
  ...
  async loadDate(_date: string): Promise<RefreshResult | null> {
    // P5 will load historical snapshots; for MVP just always return latest.
